...
public class Store {
    ...
    public static int[] buttonID ={Value.gunTower,Value.lazerTower,Value.nullTower,Value.nullTower,Value.nullTower,Value.nullTower,Value.nullTower,Value.nullTower} ;
    ...
    private void  chooceTower(int i,int j,int a){
       if (a==0){
                Tower tower=new GunTower((Screen.room.blocks[i][j]));
                Screen.room.blocks[i][j]=tower;
                Screen.room.towers.add(tower);
            }
            if(a==2) {

                Tower tower=new lazerTower((Screen.room.blocks[i][j]));
                Screen.room.blocks[i][j]=tower;
                Screen.room.towers.add(tower);
            }
    }
